...
elif [ "$command" = "fluidanimate" ]; then
...
eval pin -t ~/Downloads/parsec-3.0/advcomparch-ex1-helpcode/pintool/obj-intel64/simulator.so -o my_output.out "-L1c" "$L1c" -L1a "$L1a" -L1b "$L1b" -L2c "$L2c" -L2a "$L2a" -L2b "$L2b" -TLBe "$TLBe" -TLBa "$TLBa" -TLBp "$TLBp" -L2prf "$L2prf" -- "$prompt";
